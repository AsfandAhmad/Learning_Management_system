...
export async function getAllStudents(req, res, next) {
  try {
    const { status } = req.query; // Filter by status: Active, Blocked
    let query = "SELECT StudentID, FullName, Email, Status, CreatedAt, StreakCount, LastActiveDate FROM Student";
    const params = [];
...
export async function getStudentDetails(req, res, next) {
  try {
    ...
    const [student] = await pool.query(
      `SELECT StudentID, FullName, Email, Status, CreatedAt, StreakCount, LastActiveDate 
       FROM Student 
       WHERE StudentID = ?`,
      [studentId]
    );
    ...
    const [enrollments] = await pool.query(
      "SELECT COUNT(*) as EnrollmentCount FROM Enrollment WHERE StudentID = ?",
      [studentId]
    );

    // Get completed courses count
    const [completed] = await pool.query(
      "SELECT COUNT(*) as CompletedCount FROM Enrollment WHERE StudentID = ? AND Status = 'Completed'",
      [studentId]
    );

    return res.json({
      student: student[0],
      enrollmentCount: enrollments[0].EnrollmentCount,
      completedCount: completed[0].CompletedCount
    });
  } catch (e) {
    next(e);
  }

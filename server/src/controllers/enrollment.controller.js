...
// POST enroll student in course from body (alternative endpoint)
export async function enrollStudentFromBody(req, res, next) {
    try {
        ...
        // Verify course exists (allow Draft courses for testing)
        const [course] = await pool.query(
            "SELECT CourseID, Status FROM Course WHERE CourseID = ?",
            [courseId]
        );

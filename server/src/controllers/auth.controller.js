...
export async function loginStudent(req, res, next) {
  try {
    ...
    // Get student with retry logic
    let rows;
    let retries = 3;
    while (retries > 0) {
      try {
        [rows] = await pool.query("SELECT * FROM Student WHERE Email = ?", [email]);
        break;
      } catch (err) {
        retries--;
...
export async function loginTeacher(req, res, next) {
  try {
    ...
    // Get teacher with retry logic
    let rows;
    let retries = 3;
    while (retries > 0) {
      try {
        [rows] = await pool.query("SELECT * FROM Teacher WHERE Email = ?", [email]);
        break;
      } catch (err) {
        retries--;
        if (retries === 0 || (err.code !== 'ECONNRESET' && err.code !== 'PROTOCOL_CONNECTION_LOST')) {
